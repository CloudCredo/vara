...
module Vara
  describe 'Given a Stemcell Resource Manager' do
    ...
    let(:valid_stemcell) do
      File.join(assets_path, 'bosh_mediator', 'stemcells', 'bosh-stemcell-vsphere-0.7.0.tgz')
    end

    let(:broken_stemcell) do
      File.join(assets_path, 'bosh_mediator', 'stemcells', 'broken.tgz')
    end
    ...
    context 'When attempting to download a stemcell' do
      context 'when the file exists locally already' do
        ...
        it 'downloads a stemcell successfully from a URL' do
          ...
          expect_any_instance_of(::Net::HTTP).to receive(:request_get).and_yield(stubbed_response)
        ...
        it 'downloads a stemcell successfully from a URL' do
          ...
          begin
            ...
            expect_any_instance_of(::Net::HTTP)
              ...
              .with("/#{stemcell_file_name}", {})
              .and_yield(stubbed_response)
    ...
    context 'When interrogating stemcell metadata' do
      ...
      it 'raises an error when interrogating an invalid stemcell' do
        error_message = "The provided uri #{broken_stemcell} does not point to a valid stemcell resource"
        expect do
